...
export default function AdminOrderDetailsPage() {
  ...
  const activeIndex = useMemo(() => flow.indexOf((order?.orderStatus as typeof flow[number]) ?? 'PENDING'), [order?.orderStatus]);
    ...
    <div className="max-w-7xl mx-auto text-white space-y-6">
      <div>
        ...
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-bold font-display">Order #{order.id}</h1>
        ...
        <div className="flex justify-between items-center">
          {flow.map((step, index) => (
            <div key={step} className="flex-1 text-center">
              ...
              <p className={`mt-2 text-sm font-semibold ${index <= activeIndex ? 'text-brand-300' : 'text-gray-400'}`}>{step.replace('_', ' ')}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-surface-dark border border-surface-border rounded-2xl p-6">
            <h2 className="font-semibold text-base mb-4">Customer Details</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-gray-400">Name</p>
                <p className="font-semibold">Customer #{order.id}</p>
              </div>
              <div>
          ...
          <div className="bg-surface-dark border border-surface-border rounded-2xl p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-semibold inline-flex items-center gap-2"><Clock3 size={16} /> {format(new Date(order.pickupTime), 'dd MMM yyyy, HH:mm')}</p>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-surface-dark border border-surface-border rounded-2xl p-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-400">Subtotal</span><span>₹{order.totalAmount}</span></div>
              <div className="flex justify-between"><span className="text-gray-400">Tax</span><span>₹0</span></div>
              <div className="border-t border-surface-border pt-2 flex justify-between font-bold text-lg"><span>Total</span><span className="text-brand-400">₹{order.totalAmount}</span></div>
            </div>
          </div>
